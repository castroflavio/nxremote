...
class NXPyroSSH:
    ...
    def getOutput(self):
        ...
        try:
            ...
            raise NXPyroError("Could not retrieve output!")
        return result
    ...
    def getURIfromQueue(self):
        try:
            ...
            raise NXPyroError("Could not start Pyro service!")
    ...
    def msg(self, m):
        print "NXPyroSSH(%i) %s: %s" % (self.id, self.account(), str(m))
    ...
    def dbg(self, m):
        if self.debug == True:
            print "NXPyroSSH(%i) %s: DBG: %s" % (self.id, self.account(), str(m))
