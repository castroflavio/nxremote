...
from .ssh import NXPyroSSH

class NXPyroSession(object):
    '''
    ...
    def run(self):
        ...
        self.sshService = NXPyroSSH(self.user, self.hostname,
                                     command=command, getURI=True)
        uri = self.sshService.getURIfromQueue()
        if (uri == "UNSET"):
            print("SSH could not start NXpyro service!")
            return False
        tokens = uri.split(":")
        port = int(tokens[2])
        self.sshTunnel = NXPyroSSH(self.user, self.hostname, 
